# Энциклопедический GraphRAG: архитектура и ТЗ (MVP)

## 1. Цели, предметная область, ограничения MVP

### 1.1 Цели
- Построить MVP-систему **GraphRAG для энциклопедического корпуса** (статьи, справочные тексты, богатые сущностями документы).
- Реализовать гибридный retrieval: `BM25 + GNN` (+ dense similarity).
- Внедрить многоуровневую дедупликацию (exact + semantic + graph-context).
- Обеспечить обязательную атрибуцию источников до уровня `doc/page/span` (или эквивалентных полей provenance для веб-статей).
- Поддержать **потоковую загрузку** и обогащение через **очередь событий (Kafka)** между этапами ingestion → индекс → векторы → граф.

### 1.2 Предметная область и данные
- Фокус — **энциклопедический RAG**: ответы с опорой на фрагменты статей, перекрёстные сущности и связи в графе знаний.
- Базовый сценарий данных — **корпус статей из Hugging Face** (в т.ч. линейки **Wikimedia / Wikipedia**): заголовок, URL, текст, многоязычность; чанкинг сохраняет ссылку на исходную статью.
- Извлечение структуры графа: **NER** + **связи через LLM** (гибридный экстрактор), чтобы зафиксировать не только упоминания сущностей, но и отношения между ними.
- Система должна быть **объяснимой и трассируемой** (скоры, цитаты, при необходимости — внешняя трассировка запросов).
- Дубликаты, зеркала и пересмотренные версии статей ухудшают полноту и точность выдачи — учёт версий и каноничности остаётся в модели данных.

### 1.3 Ограничения MVP
- Масштаб: до `1 000 000` чанков.
- Языки: `RU + EN` (и другие при наличии в датасете).
- Источники: в прототипе — **поток из HF**; форматы `PDF`, `DOCX`, `HTML` остаются целевыми для расширения.
- OCR сканов вне MVP.
- Локальный запуск через `Docker Compose` (см. `deploy/services`, `deploy/infrastructure`).
...
2. `Indexing / graph workers`
- Парсинг и чанкинг (в т.ч. **Stage2**), извлечение сущностей (**NER**) и связей (**LLM**), дедупликация и каноникализация.
- Запись в поисковый индекс (OpenSearch/Elasticsearch) и Neo4j.

3. `OpenSearch / Elasticsearch (Evidence/Text Store)`
...
6. `Queue` (**Apache Kafka**)
- Асинхронная обработка этапов: сообщения чанков, векторизация, задачи на построение/обновление графа.
- Развязка **Stage2 (ingestion)** и **Stage3** (индекс, эмбеддинги, Neo4j).

7. `Object Storage (логический компонент MVP)`
- Оригинальные файлы и служебные артефакты.

### 2.2 Поток индексации
1. **Ingestion (Stage2)** читает датасет, нормализует текст, выполняет чанкинг и **публикует сообщения в Kafka** (топик чанков).
2. **Stage3_1** потребляет чанки, индексирует текст в поисковом хранилище, публикует события векторизации.
3. **Stage3_2** строит эмбеддинги и пишет векторы в Qdrant (и/или в Neo4j в `neo4j_native`), публикует события для графа.
4. **Stage3_3** извлекает сущности (**NER**) и связи (**LLM**), обновляет Neo4j.
5. Опционально: REST `POST /index/documents` создаёт `job_id` и публикует задачи в очередь (альтернативный контур).
6. Выполняется дедуп (exact -> semantic -> graph-context) на соответствующих этапах.
7. Запись:
- в OpenSearch/Elasticsearch: чанки + provenance (`doc/page/span`),
- в Neo4j: сущности и отношения; в `neo4j_native` дополнительно `Chunk`-узлы с `embedding` и `gnn_embedding`,
- в Qdrant: `text embeddings` и `entity embeddings` с version-тегами.
8. `GET /index/jobs/{job_id}` отдаёт статус и дедуп-статистику (при использовании REST-контура индексации).
...
### 3.1 OpenSearch / Elasticsearch (текст и доказательная база)
Индекс `legal_chunks` (имя в коде; **семантически** — индекс энциклопедических чанков с provenance):
- `chunk_id` (keyword)
- `doc_id` (keyword)
- `version_id` (keyword)
- `section_id` (keyword)
...
Назначение: запуск батч-индексации (альтернатива полностью Kafka-driven Stage2).
...
```json
{
  "documents": [
    {
      "external_id": "wiki-article-456",
      "source_type": "html",
      "language": "en",
      "title": "Graph neural network",
      "content_uri": "https://example.org/wiki/Graph_neural_network",
      "metadata": {"dataset": "wikimedia", "url": "https://..."}
    }
  ]
}
```
...
```json
{
  "query": "Как связаны ранний социализм и анархизм в европейской истории?",
  "language": "ru",
  "filters": {
    "language": "ru",
    "is_canonical": true
...
- Docker Compose: API, воркеры, OpenSearch/Elasticsearch, Neo4j, Qdrant, **Kafka**, при необходимости объектное хранилище.
- Health checks и конфигурация окружения (см. `deploy/services`, `deploy/infrastructure`).
...
- Меры: регрессионный benchmark перед релизом, canary-набор **энциклопедических** запросов (факты, связи сущностей, определения).
...
- Стек: Python 3.13, OpenSearch или Elasticsearch, Neo4j, Qdrant, Kafka, Docker Compose.
- Embeddings и LLM: в типовом развёртывании — **Ollama** (или иной совместимый endpoint); возможна интеграция с облачными API.
- OCR не входит в MVP.
...
- Легче реализовать explainability графовых связей в энциклопедических сценариях (сущности и их соседство).
...
- `Memgraph`: высокая производительность, но меньше зрелых практик/инструментов именно для mixed encyclopedic KG workflows.
...
### 11.2 OpenSearch / Elasticsearch (вместо PostgreSQL FTS / только vector DB)
Преимущества для данного MVP:
- Надежный BM25 из коробки для **лексического** keyword retrieval по большому корпусу статей.
...
- Только vector DB: недостаточно для сильного lexical recall по редким терминам и именам, где BM25 критичен.
